...
router.get('/getMetricExpressions', async (req, res) => {
  try {
    const queryResult = await db.query('SELECT id, metricName, metricExpression FROM metricsExpressions');
...
router.put('/updateMetricExpression/:id', async (req, res) => {
  ...
  try {
    const queryResult = await db.query('UPDATE metricsExpressions SET metricExpression = $1 WHERE id = $2 RETURNING id, metricExpression', [metricExpression, id]);
    ...
    if (updatedMetricDetails) {
      ...
      res.status(400).json({ statusCode: 400, errorMessage: 'Metric not found' });
    }
  } catch (error) {
